...
Построение точной (BREP, не полигональной) геометрии реактора
средствами CadQuery/OpenCASCADE и экспорт в STEP.

STL/VTK работает с треугольными сетками -- это нормально для CFD,
но не подходит для точного STEP-представления (STEP должен нести
точную геометрию граней/рёбер, а не аппроксимацию треугольниками).
Поэтому для STEP используется отдельный бэкенд на базе
CadQuery (обёртка над OpenCASCADE), где те же боксы строятся как
точные BREP-примитивы и объединяются булевой операцией fuse.
"""
...
def build_main_body_solid(p: TReactorParams) -> cq.Workplane:
    """Строит точное (BREP) тело канала выдавливанием Т-образного
    контура -- тем же геометрическим методом, что и STL-бэкенд
    (см. vtk_backend.build_main_body_polydata), для полной
    консистентности STL- и STEP-представлений.
    """
